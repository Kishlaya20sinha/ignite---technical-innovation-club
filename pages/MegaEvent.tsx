...
const MegaEvent: React.FC = () => {
    const { slug } = useParams<{ slug: string }>();
    const navigate = useNavigate();
    const [megaEvent, setMegaEvent] = useState<EventType | null>(null);
    ...
    useEffect(() => {
        const fetchEvent = async () => {
            setLoadingPage(true);
            try {
                const allEvents = await api.getEvents(); // Or getAllEvents if getEvents is filtered
                // Find event matching the slug
                ...
                if (foundEvent) {
                    setMegaEvent(foundEvent);
                    // Filter sub-events for this mega event
                    const subs = allEvents.filter((e: any) => e.parentId === foundEvent._id);
                    setSubEvents(subs);
                } else {
    ...
    if (!megaEvent) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center text-white space-y-4">
                ...
                <button onClick={() => navigate('/events')} className="px-6 py-2 bg-primary rounded-full">Go to Events</button>
            </div>
        );
    }
                ...
                <SectionWrapper>
                    <div className="text-center mb-16">
                        ...
                        <p className="text-gray-400 text-lg max-w-xl mx-auto">
                            {megaEvent.description || "Compete, innovate, and showcase your skills."}
                        </p>
                    </div>
                </SectionWrapper>

                {!selectedEvent ? (
                    /* Event Cards for Sub-Events */
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                        {subEvents.length === 0 && (
                            <div className="col-span-2 text-center py-20 text-gray-500">
                                <p className="text-lg">Sub-events will be announced soon!</p>
